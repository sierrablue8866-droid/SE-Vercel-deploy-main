/**
 * ═══════════════════════════════════════════════════════════════════════════
 *  Sierra Estates — Firebase CRUD Utilities (Admin SPA)
 *  File: SE/apps/admin/src/services/firebaseUtils.ts
 * ═══════════════════════════════════════════════════════════════════════════
 *
 *  Modular Firestore CRUD functions for the Admin SPA.
 *
 *  🏗️ Low-Cost Firebase Strategy:
 *    - NO Cloud Functions. All logic runs client-side in the Admin SPA.
 ...
 *  Reads config from Vite env vars (VITE_FIREBASE_*) — never hardcode.
 * ────────────────────────────────────────────────────────────────────────── */
...
function getDb(): Firestore {
  ...
  if (!firebaseConfig.apiKey || !firebaseConfig.projectId) {
    throw new Error(
      'Firebase config missing. Set VITE_FIREBASE_* env vars in apps/admin/.env'
    );
  }
 ...
 *
 * @example
 *   const result = await createListingWithOwner({
 *     listing: {
 *       status: 'active',
 *       property_type: 'villa',
 ...
 *     },
 *   });
 *   console.log(result.listingId); // == result.ownerId
 */
...
/**
 * Single import surface for the Admin SPA:
 *   import { fetchListings, createListingWithOwner, ... } from '@/services/firebaseUtils';
 */
