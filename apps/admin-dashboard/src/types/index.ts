...
export interface Listing {
  ...
  /** Whether a virtual tour is available. */
  vr_available?: boolean;

  /** URL to the VR tour hosted on Firebase or external provider. */
  vr_link?: string;
...
export interface ChatMessage {
  ...
  sender: 'client' | 'bot' | 'agent' | 'user' | 'ai';
  /** Message text. */
  text: string;
...
export interface Workflow {
  ...
  desc?: string;
  descAr?: string;
  status: 'active' | 'warning' | 'paused' | string;
  runs?: number;
  last?: string;
  color?: string;
  createdAt?: Date;
