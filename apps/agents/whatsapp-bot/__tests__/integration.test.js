...
import { classifyIntent, routeMessage, determineUrgency, WhatsAppBotRouter, } from '../router'

// ── Mock AgentOrchestrator ───────────────────────────────────────────────────
jest.mock('@sierra-estates/agents-core', () => ({
  AgentOrchestrator: jest.fn().mockImplementation(() => ({
    runAgentTask: (...args) => (global ).mockRunAgentTask(...args),
...
(global ).mockRunAgentTask = mockRunAgentTask
...
describe('Integration: Full WhatsApp Message Flow', () => {
  ...
  beforeEach(() => {
    ...
    mockRunAgentTask.mockImplementation((agentName) => {
      if (agentName === 'hermes') {
  ...
  describe('Property search flow', () => {
    it('should engage Sierra and OpenClaw for property search', async () => {
      ...
      const calledAgents = mockRunAgentTask.mock.calls.map((c) => c[0])
    ...
    it('should inject live listings into the Hermes context for property searches', async () => {
      ...
      const hermesCall = mockRunAgentTask.mock.calls.find((c) => c[0] === 'hermes')
      expect(hermesCall).toBeTruthy()
      const hermesContext = String(_nullishCoalesce(hermesCall[2], () => ( '')))
...
describe('Intent → Route → Urgency pipeline', () => {
  ...
  scenarios.forEach(({ body, expectedIntent, expectedPrimary }) => {
    it(`"${body.substring(0, 30)}" → intent=${expectedIntent}, route=${expectedPrimary}`, () => {
      ...
      const urgency = determineUrgency(intent , [])
      const route = routeMessage(intent , urgency, false)
