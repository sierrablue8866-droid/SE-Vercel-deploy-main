 function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }// ⚠️  dotenv MUST be loaded before any module that reads process.env at import time
...
async function checkStoredLead(phoneStr) {
  try {
    ...
    const matches = await listRecords('inquiries', {
...
client.on('message', async (msg) => {
  try {
    ...
    if (isAdmin && msg.body.trim().startsWith('!whitelist')) {
      ...
      const cmd = _optionalChain([parts, 'access', _ => _[1], 'optionalAccess', _2 => _2.toLowerCase, 'call', _3 => _3()]);
