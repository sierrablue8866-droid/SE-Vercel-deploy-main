...
import { PropertyMatchmaker, type PropertyListing } from '@sierra-estates/agents-core'
 ...
 */
export async function fetchListings(options: { limit?: number } = {}): Promise<RawListing[]> {
  ...
  try {
    ...
    const list = Array.isArray(data)
      ? (data as RawListing[])
      : (data as { listings?: RawListing[] } | null)?.listings ?? []
    return list
  } catch (err) {
 ...
 */
export function extractClientProfile(message: string): {
  ...
  if (budgetMatch) {
    ...
    if (value > 100) {
      profile.budgetMax = Math.round(value * 1_000_000 / 50) // EGP millions → USD
    } else {
      profile.budgetMax = Math.round(value * 1_000_000) // USD millions
    }
  } else {
 ...
 */
export async function buildListingsDigest(message: string, intent: string): Promise<ListingFetchResult> {
