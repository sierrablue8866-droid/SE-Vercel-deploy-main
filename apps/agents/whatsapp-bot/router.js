...
import { buildListingsDigest, } from './property-finder'
...
export function determineUrgency(intent, history) {
...
export function routeMessage(intent, urgency, isNewClient) {
...
export class WhatsAppBotRouter {
   ...
   */
   async runCloserAgent(phone, userMessage) {
    try {
      ...
      const context = {
        dealId: dealData.id ,
        leadPhone: phone,
        propertyCode: (meta.propertyCode ) || (dealData.listing_id ) || 'N/A',
        buyerProfile: (meta.buyerProfile ) || {},
        propertyData: (meta.propertyData ) || {},
        previousOffers: (meta.previousOffers ) || [],
        negotiationHistory: (meta.negotiationHistory ) || [],
      }
...
${history.slice(-5).map((h) => JSON.stringify(h)).join('\n') || 'None'}
    `.trim()
  }
