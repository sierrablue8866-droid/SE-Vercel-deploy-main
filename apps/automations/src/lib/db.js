 ...
 */
import { getSupabaseAdmin } from '../../../../packages/db/lib/index.js';

export { insertRecord, listRecords, updateRecord } from '../../../../packages/db/lib/index.js';
 ...
 */
export function assertDbConfigured(label) {
  try {
    getSupabaseAdmin();
    return true;
  } catch (error) {
    console.warn(
      `[${label}] Supabase is not configured: ${(error ).message} ` +
