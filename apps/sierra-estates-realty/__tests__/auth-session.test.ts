/**
 * Tests: lib/auth.ts — session signing/verification and role enforcement.
 *
 * The `sierra_sess` cookie is the whole admin-portal credential: it carries a
 * role and is trusted by every route that calls `requireRole`. So the
 * load-bearing properties are (a) a payload cannot be edited without
 * invalidating the signature, and (b) the signing key is never a value an
 * attacker can know.
 *
 * NOTE: lib/auth.ts captures IS_PROD at import time, so the production-mode
 * assertions re-import via `jest.resetModules()`.
 */
...
function cookieRequest(token: string | null): Request {
...
describe('signSession / verifySession', () => {
  ...
  it('rejects a token whose payload was tampered with', async () => {
    ...
    const token = await signSession({ ...SESSION, role: 'viewer' as Role });
...
describe('requireRole', () => {
  ...
  async function statusFor(role: Role, min: Role): Promise<number | 'ok'> {
  ...
  it('throws a 401 Response when there is no session at all', async () => {
    ...
    try {
      ...
      await expect((thrown as Response).json()).resolves.toEqual({ error: 'Unauthorized' });
    }
  });
  ...
  it('rejects a session whose role is not a known role at all', async () => {
    await expect(statusFor('root' as Role, 'viewer')).resolves.toBe(403);
    await expect(statusFor('' as Role, 'admin')).resolves.toBe(403);
  });
