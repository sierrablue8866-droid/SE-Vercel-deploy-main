...
function post(body, url = 'https://sierra-estates.net/api/x') {
...
function malformed(url = 'https://sierra-estates.net/api/x') {
  return new Request(url, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: '{not json',
...
describe('POST /api/roi/calculate', () => {
  it('computes gross, net, five-year and payback for a standard input', async () => {
    ...
    const res = await roiCalculate(post({ price: 10000000, rent: 100000, appreciation: 15 }));
    ...
    const expectedFiveYr =
      ((1200000 * 5 * 0.82 + 10000000 * (Math.pow(1.15, 5) - 1)) / 10000000) * 100;
  ...
  it('scales the yield down as price rises', async () => {
    const cheap = await (await roiCalculate(post({ price: 5000000, rent: 50000 }))).json();
    const dear = await (await roiCalculate(post({ price: 20000000, rent: 50000 }))).json();
  ...
  it('never divides by zero on payback, even at zero net yield', async () => {
    ...
    const body = await (await roiCalculate(post({ price: 10000000, rent: 0.0000001 }))).json();
  ...
  it('rejects a price of 0 instead of silently substituting 10,000,000', async () => {
    const res = await roiCalculate(post({ price: 0, rent: 100000 }));
  ...
  it('rejects a negative price', async () => {
    const res = await roiCalculate(post({ price: -5, rent: 100000 }));

    expect(res.status).toBe(400);
  });

  it('honours a rent of 0 as a genuine zero yield', async () => {
    const body = await (await roiCalculate(post({ price: 10000000, rent: 0 }))).json();
  ...
  it('rejects a negative rent', async () => {
    const res = await roiCalculate(post({ price: 10000000, rent: -1 }));

    expect(res.status).toBe(400);
  });

  it('honours 0% appreciation instead of substituting 15%', async () => {
    const zero = await (await roiCalculate(post({ price: 10000000, rent: 100000, appreciation: 0 }))).json();
    const fifteen = await (await roiCalculate(post({ price: 10000000, rent: 100000, appreciation: 15 }))).json();
  ...
  it('supports negative appreciation (depreciation)', async () => {
    const body = await (
      await roiCalculate(post({ price: 10000000, rent: 100000, appreciation: -10 }))
    ).json();
  ...
  it('rejects a non-numeric price rather than substituting a default', async () => {
    const res = await roiCalculate(post({ price: 'abc', rent: 100000 }));
  ...
  it('still defaults a genuinely absent field', async () => {
    const body = await (await roiCalculate(post({ rent: 100000 }))).json();
...
describe('POST /api/pricing/evaluate', () => {
  it('values a listing from the zone rate, area, and bedroom premium', async () => {
    ...
    expect(body.value).toBeCloseTo(12400000, 10);
    expect(body.rangeLow).toBeCloseTo(12400000 * 0.93, 10);
    expect(body.rangeHigh).toBeCloseTo(12400000 * 1.07, 10);
  });
