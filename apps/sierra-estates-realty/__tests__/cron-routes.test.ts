...
function request(headers: Record<string, string> = {}): NextRequest {
  return new NextRequest('https://sierra-estates.net/api/cron/x', { headers } as any);
}
...
const routes: Array<[string, (req: NextRequest) => Promise<Response>]> = [
  ['sync-leads', syncLeads as never],
  ['sync-listings', syncListings as never],
  ['maintenance', maintenance as never],
  ['sync-master-sheet', syncMasterSheet as never],
];
