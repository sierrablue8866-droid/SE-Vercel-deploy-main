...
jest.mock('@/lib/inventory/gazetteer', () => ({
  resolveLocation: (...args) => resolveLocation(...args),
}));
...
describe('status mapping', () => {
  it.each([
    ...
    const result = queryUnitToMapUnit(queryUnit({ status: source  }));
  ...
  it('falls back to off-market for an unrecognised status', () => {
    const result = queryUnitToMapUnit(queryUnit({ status: 'pending'  }));
...
describe('rent vs sale heuristic', () => {
  it('treats a sub-1M price as rent', () => {
    expect(queryUnitToMapUnit(queryUnit({ price: 45000 })).mode).toBe('rent');
    expect(queryUnitToMapUnit(queryUnit({ price: 999999 })).mode).toBe('rent');
  });

  it('treats 1M and above as sale', () => {
    expect(queryUnitToMapUnit(queryUnit({ price: 1000000 })).mode).toBe('sale');
    expect(queryUnitToMapUnit(queryUnit({ price: 28500000 })).mode).toBe('sale');
  });
...
describe('price labelling', () => {
  it('renders a whole-million price with no decimal', () => {
    expect(queryUnitToMapUnit(queryUnit({ price: 28000000 })).priceLabel).toBe('EGP 28M');
  });

  it('renders a partial million to one decimal', () => {
    expect(queryUnitToMapUnit(queryUnit({ price: 28500000 })).priceLabel).toBe('EGP 28.5M');
  });

  it('renders sub-million prices with thousands separators', () => {
    expect(queryUnitToMapUnit(queryUnit({ price: 45000 })).priceLabel).toBe('EGP 45,000');
  });
...
describe('coordinates', () => {
  it('prefers the unit\'s own coordinates and marks the location exact', () => {
    const unit = { ...queryUnit(), coordinates: { lat: 30.1, lng: 31.5 } } ;
...
describe('field passthrough and null-coalescing', () => {
  ...
  it.each(['code', 'propertyType'] )('nulls an empty %s rather than emitting ""', (field) => {
    const result = queryUnitToMapUnit(queryUnit({ [field]: '' } ));
