...
import type { PFListingRequest } from '../lib/property-finder/types';

/** A minimal listing that passes every rule; tests override single fields. */
function validListing(overrides: Partial<PFListingRequest> = {}): PFListingRequest {
  return {
    ...
    price: { type: 'sale', amounts: { sale: 28_500_000 } },
    ...overrides,
  } as PFListingRequest;
}
...
describe('EgyptListingValidator — required fields', () => {
  it.each([
    ...
    const result = EgyptListingValidator.validate(
      validListing({ [field]: undefined } as Partial<PFListingRequest>),
    );
  ...
  it('reports a missing location id', () => {
    const result = EgyptListingValidator.validate(
      validListing({ location: undefined } as Partial<PFListingRequest>),
    );
  ...
  it.each([[0], [-5], [undefined]])('reports an invalid size: %s', (size) => {
    const result = EgyptListingValidator.validate(
      validListing({ size } as Partial<PFListingRequest>),
    );
...
describe('EgyptListingValidator — title and description', () => {
  it('requires an English title of at least 10 characters', () => {
    ...
    expect(
      EgyptListingValidator.validate(validListing({ title: undefined } as Partial<PFListingRequest>))
        .errors,
    ).toContain(message);
  });
...
describe('EgyptListingValidator — media', () => {
  it('requires at least one image', () => {
    ...
    expect(
      EgyptListingValidator.validate(validListing({ media: undefined } as Partial<PFListingRequest>))
        .errors,
    ).toContain('At least one image is required.');
  });
  ...
  it('requires at least one image to carry an original URL', () => {
    const result = EgyptListingValidator.validate(
      validListing({ media: { images: [{ original: {} }] } } as Partial<PFListingRequest>),
    );
...
describe('EgyptListingValidator — price', () => {
  it('requires price information', () => {
    const result = EgyptListingValidator.validate(
      validListing({ price: undefined } as Partial<PFListingRequest>),
    );
  ...
  it('requires the amount matching the price type to be positive', () => {
    const result = EgyptListingValidator.validate(
      validListing({ price: { type: 'sale', amounts: { sale: 0 } } } as Partial<PFListingRequest>),
    );
  ...
  it('reports a price type with no matching amount', () => {
    ...
    const result = EgyptListingValidator.validate(
      validListing({ price: { type: 'rent', amounts: { sale: 100 } } } as unknown as Partial<PFListingRequest>),
    );
