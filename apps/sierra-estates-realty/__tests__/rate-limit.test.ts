...
type RateLimitModule = typeof import('../lib/server/rate-limit');

/** Re-import the module with Upstash either configured or absent. */
async function loadLimiter(opts: { upstash: boolean }): Promise<RateLimitModule> {
  if (opts.upstash) {
...
function upstashOk(count: number) {
  return {
    ok: true,
    status: 200,
...
describe('in-memory limiter', () => {
  it('allows requests up to the limit and blocks the one after', async () => {
    ...
    const limiter = createRateLimiter({ name: 't', windowMs: 60_000, maxRequests: 3 });
  ...
  it('sets a Retry-After header of at least 1 second', async () => {
    const { createRateLimiter } = await loadLimiter({ upstash: false });
    const limiter = createRateLimiter({ name: 't', windowMs: 60_000, maxRequests: 1 });
  ...
  it('does not call fetch when Upstash is not configured', async () => {
    const { createRateLimiter } = await loadLimiter({ upstash: false });
    const limiter = createRateLimiter({ name: 't', windowMs: 60_000, maxRequests: 5 });
...
describe('Upstash limiter', () => {
  it('allows a request when INCR is within the limit', async () => {
    ...
    (global.fetch as jest.Mock).mockResolvedValueOnce(upstashOk(1));
    const limiter = createRateLimiter({ name: 'pub', windowMs: 60_000, maxRequests: 30 });
  ...
  it('blocks with 429 once INCR exceeds the limit', async () => {
    const { createRateLimiter } = await loadLimiter({ upstash: true });
    (global.fetch as jest.Mock).mockResolvedValueOnce(upstashOk(31));
    const limiter = createRateLimiter({ name: 'pub', windowMs: 60_000, maxRequests: 30 });
  ...
  it('still enforces the limit through the in-memory fallback during an outage', async () => {
    ...
    const limiter = createRateLimiter({ name: 'pub', windowMs: 60_000, maxRequests: 2 });
