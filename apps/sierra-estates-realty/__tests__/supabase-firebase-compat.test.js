 function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }import { createFirestoreCompat, createAuthCompat, createStorageCompat } from '@sierra-estates/db';
import { MemoryPalace } from '../../../packages/memory-engine/src/index.js';
import fs from 'fs';
...
describe('Supabase - Firebase Compatibility Layer & Memory Transition', () => {
  ...
  describe('System Memory Verification (Supabase as Authoritative Backend)', () => {
    it('should have Supabase declared as primary authoritative backend in MemoryPalace', () => {
      ...
      expect(_optionalChain([entry, 'optionalAccess', _ => _.metadata, 'optionalAccess', _2 => _2.provider])).toBe('supabase');
      expect(_optionalChain([entry, 'optionalAccess', _3 => _3.metadata, 'optionalAccess', _4 => _4.status])).toBe('active-primary');
      expect(_optionalChain([entry, 'optionalAccess', _5 => _5.metadata, 'optionalAccess', _6 => _6.replaces])).toBe('firebase');
      expect(_optionalChain([entry, 'optionalAccess', _7 => _7.content])).toContain('Supabase is the primary authoritative backend');
    });
