...
describe('GET /api/wealth/portfolio', () => {
  ...
  test('returns curated portfolio with default count and undefined market', async () => {
    ...
    (WealthService.getCuratedPortfolio ).mockResolvedValue(portfolio);
  ...
  test('passes through query parameters for count and market', async () => {
    (WealthService.getCuratedPortfolio ).mockResolvedValue([]);
  ...
  test('returns 500 when service throws', async () => {
    (WealthService.getCuratedPortfolio ).mockRejectedValue(new Error('portfolio failure'));
