...
export async function GET(req) {
  ...
  try {
    ...
    let query = adminDb.collection('followups');
    ...
    if (authResult.method === 'firebase' && authResult.uid) {
      ...
      const role = _optionalChain([callerDoc, 'access', _ => _.data, 'call', _2 => _2(), 'optionalAccess', _3 => _3.role]);
...
export async function POST(req) {
  ...
  try {
    ...
    const agentId = _nullishCoalesce(_nullishCoalesce(parsed.data.agentId, () => ( authResult.uid)), () => ( 'system'));
    ...
    const ref = await adminDb.collection('followups').add({
      ...
      createdBy: _nullishCoalesce(authResult.uid, () => ( 'system')),
    });
