...
export async function GET(req) {
  ...
  try {
    ...
    const records = await listRecords('search_queries', {
    ...
    const avgResultsPerQuery =
      totalQueries > 0
        ? records.reduce((sum, r) => sum + (_nullishCoalesce(r.resultCount, () => ( 0))), 0) / totalQueries
        : 0;
    const noResultRate =
      totalQueries > 0
        ? records.filter((r) => r.resultCount === 0).length / totalQueries
        : 0;
