...
export async function POST(request) {
  ...
  try {
    ...
    const matchedUnit = await _asyncNullishCoalesce(await _asyncNullishCoalesce(unit, async () => ( (await InventoryQueryService.query({
    ...
    const contactName = _optionalChain([matchedUnit, 'optionalAccess', _ => _.ownerContact])
      ? (isDirectOwner ? 'Owner' : 'Broker')
      : isDirectOwner ? 'Owner (from Master Sheet)' : 'Broker (from Master Sheet)';
    const contactPhone = _optionalChain([matchedUnit, 'optionalAccess', _2 => _2.ownerContact]) || (isDirectOwner ? '+20 100 987 6543' : '+20 122 345 6789');

    const compound = _optionalChain([matchedUnit, 'optionalAccess', _3 => _3.compound]) || _optionalChain([matchedUnit, 'optionalAccess', _4 => _4.location]) || 'New Cairo';
    const priceStr = _optionalChain([matchedUnit, 'optionalAccess', _5 => _5.price])
      ? `${(matchedUnit.price / 1000000).toFixed(1)}M EGP`
      : 'السعر عند التواصل';
