  ...
  try {
    ...
    for (const jobDoc of queued.docs) {
      ...
      try {
        ...
        await jobDoc.ref.update({
          status: 'failed',
          errorMessage: _optionalChain([err, 'optionalAccess', _ => _.message]) || String(err),
          attempts: (_nullishCoalesce(job.attempts, () => ( 0))) + 1,
          updatedAt: Timestamp.now(),
        });
        failed++;
    ...
    return NextResponse.json(
      { success: false, error: _optionalChain([error, 'optionalAccess', _2 => _2.message]) || 'dispatch failed' },
      { status: 500 },
    );
  }
}
