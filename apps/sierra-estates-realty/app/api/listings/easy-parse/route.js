...
function heuristicFallbackParse(rawText) {
  ...
  // Detect Mode
  const mode = (lower.includes('rent') || lower.includes('ايجار') || lower.includes('إيجار') || lower.includes('للايجار')) ? 'rent' : 'sale';
  ...
  if (priceNumberMatch) {
    ...
    if (millions > 0 && millions < 500) {
      price = Math.round(millions * 1000000);
    }
  }
  ...
  return {
    ...
    sierraCode: _optionalChain([codeMeta, 'optionalAccess', _ => _.code]) || `SE-${detectedCompound.slice(0,3).toUpperCase()}-${Date.now().toString().slice(-4)}`,
    aiScore: 9.4,
...
Return STRICTLY a JSON object with this format (no markdown code fences):
{
  ...
  "confidence": number (between 0.8 and 1.0)
        ...
        const result = {
          ...
          sierraCode: _optionalChain([codeMeta, 'optionalAccess', _2 => _2.code]) || `SE-AIR-${Date.now().toString().slice(-4)}`,
          aiScore: 9.6,
    ...
    return NextResponse.json(
      { success: false, error: _optionalChain([error, 'optionalAccess', _3 => _3.message]) || 'Failed to parse listing' },
      { status: 500 }
    );
  }
}
