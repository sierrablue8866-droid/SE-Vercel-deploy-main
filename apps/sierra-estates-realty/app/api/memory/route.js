...
import { memoryEngine, sharedMemory, openMemoryClient } from '../../../../../packages/memory-engine/src/index.js';
...
export async function POST(req) {
  ...
  try {
    ...
    if (data !== undefined) {
      let dataBytes = 0;
      try {
        dataBytes = Buffer.byteLength(_nullishCoalesce(JSON.stringify(data), () => ( '')), 'utf8');
      } catch (e) {
        return NextResponse.json(
    ...
    if (action === 'store_semantic' || (content && !action)) {
      ...
      if (key) {
        await sharedMemory.write(
          ...
          { author: (agentId ) || 'admin', tags, ttlSeconds }
        );
      }
