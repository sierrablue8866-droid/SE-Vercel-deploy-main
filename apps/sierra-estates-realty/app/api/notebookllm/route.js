...
export async function POST(req) {
  try {
    ...
    if (action === 'query') {
      ...
      const result = await engine.queryGroundedSources(sources , query, language);
      return NextResponse.json({
        success: true,
        data: result,
      });
    ...
    if (action === 'audio-overview') {
      const result = await engine.generateAudioOverview(sources , focusTopic, language);
      return NextResponse.json({
        success: true,
        data: result,
      });
    ...
    if (action === 'study-guide') {
      const result = await engine.generateStudyGuide(sources , language);
      return NextResponse.json({
        success: true,
        data: result,
      });
