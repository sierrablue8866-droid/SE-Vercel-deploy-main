...
export async function fetchListings(max = 24) {
  try {
...
;
    ...
    return body.listings.map((row) =>
      mapRow(typeof row.id === 'string' ? row.id : String(_nullishCoalesce(row.id, () => ( ''))), row)
    );
  } catch (e) {
    return [];
  }
