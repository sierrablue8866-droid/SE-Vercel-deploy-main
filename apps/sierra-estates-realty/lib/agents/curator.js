...
export const runCurator = async (
  ...
  return instrumentAgent('curator', stage, docId, async () => {
    ...
    if (stage === 'S3') {
      ...
      const mediaUrls = _optionalChain([doc, 'optionalAccess', _ => _.mediaUrls]) || [];
...
- tagline: A 3-word cinematic tagline.`;
      ...
      const userPrompt = [
        ...
        ...(mediaUrls.length > 0
          ...
          : []),
        `Brand this property: "${_optionalChain([doc, 'optionalAccess', _2 => _2.description]) || _optionalChain([doc, 'optionalAccess', _3 => _3.rawMessage]) || JSON.stringify(doc)}"`,
      ];
        ...
        if (mediaUrls.length > 0) {
          ...
          brandedMediaUrls = await Promise.all(
            sourceLimit.map((url, index) =>
              BrandingService.brandPropertyImage(docId, url, _optionalChain([doc, 'optionalAccess', _4 => _4.id]) || `UNIT-${index}`)
            )
          );
        }
...
- pf: A brief Property Finder compatible description.`;
      ...
      const userPrompt = `Generate distribution copy for: "${_optionalChain([doc, 'optionalAccess', _5 => _5.descriptionEn]) || _optionalChain([doc, 'optionalAccess', _6 => _6.description]) || ''}"`;
