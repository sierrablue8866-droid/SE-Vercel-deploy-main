 function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } } function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }// sierra-estatese/lib/integrations/portfolio-asset-registry.ts
...
export async function syncAllAssetsToRegistry() {
  const assets = await listRecords(COLLECTIONS.portfolioAssets, {
    where: [
...
export async function handleStakeholderWebhook(
  ...
  try {
    ...
    const duplicates = await listRecords(COLLECTIONS.stakeholders, {
    ...
    const assetRef = _nullishCoalesce(_optionalChain([assets, 'access', _ => _[0], 'optionalAccess', _2 => _2.id]), () => ( null));
    ...
    const newStakeholder = await insertRecord(COLLECTIONS.stakeholders, {
