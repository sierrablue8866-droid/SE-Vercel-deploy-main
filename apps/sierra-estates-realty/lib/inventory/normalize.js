...
function cell(row, key) {
  ...
  for (const k of Object.keys(row)) {
    if (k.trim().toLowerCase() === want) return String(_nullishCoalesce(row[k], () => ( ''))).trim();
  }
  return '';
}
...
export function priceLabel(price, mode) {
...
function clampPrice(price, rawMode) {
  ...
  if (price <= 0) return 0;
  if (mode === 'sale') return price > 5000000000 ? 0 : price;
  return price > 5000000 ? 0 : price;
}
