 function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } }import 'server-only';
 ...
 */
export function toListingColumns(input) {
    const columns = {};
    const extras = {};

    for (const [key, value] of Object.entries(input)) {
        if (value === undefined) continue;
        const column = _nullishCoalesce(COLUMN_ALIASES[key], () => ( key));
    ...
    if (Object.keys(extras).length > 0) {
        columns.rawData = { ...(columns.rawData ), ...extras };
    }
    return columns;
}
 ...
 */
export function toListingRecord(row) {
    const { rawData, ...rest } = row ;

    const record = { ...(_nullishCoalesce(rawData, () => ( {}))), ...rest };
