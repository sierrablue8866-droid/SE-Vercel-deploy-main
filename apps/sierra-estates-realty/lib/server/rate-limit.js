...
function tooManyRequests(resetAtMs) {
 ...
 */
function checkInMemory(key, config) {
  const now = Date.now();
 ...
 */
async function checkUpstash(key, config) {
  ...
  try {
    ...
    const count = _optionalChain([data, 'optionalAccess', _ => _[0], 'optionalAccess', _2 => _2.result]);
