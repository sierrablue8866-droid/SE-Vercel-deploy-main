...
export class OmnichannelChatService {
   ...
   */
  static async handleIncomingMessage(payload) {
    ...
    const agentResult = await processAgentCommand(numericSenderId || stakeholderId , text);
   ...
   */
   static async resolveInvestmentStakeholder(
    platform,
    senderId,
    senderName
  ) {
    ...
    const created = await insertRecord(COLLECTIONS.stakeholders, newStakeholder);
   ...
   */
   static isMessagePropertyListing(text) {
