...
export class WhatsAppParserService {
    ...
    try {
      ...
      try {
        ...
        const created = await Promise.race([
          insertRecord(COLLECTIONS.brokerListings, signal),
          new Promise((_, reject) => setTimeout(() => reject(new Error('Database timeout')), 2000)),
        ]);

        if (media && _optionalChain([created, 'optionalAccess', _2 => _2.id])) {
          try {
   ...
   */
   static async checkForDuplicates(data) {
    ...
    for (const row of _nullishCoalesce(rows, () => ( []))) {
      const existing = (row );
      const existingPrice = _optionalChain([existing, 'access', _6 => _6.extracted_data, 'optionalAccess', _7 => _7.price]) || 0;
