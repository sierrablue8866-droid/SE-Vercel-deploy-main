...
const FEATURE_PRIORITY = ['G', 'P', 'R', 'V'];
...
const normalizeLocationCode = (input) => {
  if (!input) {
    return 'UNK';
  }
...
const normalizeFurnishingStatus = (value) => {
...
const normalizeFeatureCodes = (features = []) => {
  const codes = new Set();
...
const formatPriceToken = (price, currency) => {
  if (currency === 'USD') {
...
export function buildSierraCodeMetadata(input, sourcePlatform = 'W', agentInitials = 'SB') {
 ...
 */
export function generateSierraCode(input) {
 ...
 */
export function parseSierraCode(code) {
