...
const COMMERCIAL_TYPES = ['commercial'];
...
export function normalizePropertyType(raw)
 ...
 {
  const s = String(_nullishCoalesce(raw, () => ( ''))).trim().toLowerCase();

  const matches = (...needles) => needles.some((n) => s.includes(n));
...
export function isCommercialType(t) {
...
export function normalizeListingType(raw) {
  const s = String(_nullishCoalesce(raw, () => ( ''))).trim().toLowerCase();
...
export function parseNumeric(raw) {
  ...
  s = s.replace(/[٠-٩]/g, (d) => _nullishCoalesce(ARABIC_DIGITS[d], () => ( d)));
  ...
  if (millionWord && numeric < 1000) numeric *= 1000000;
  else if (thousandWord && numeric < 1000) numeric *= 1000;
  return numeric;
}
 ...
 */
export function mapRowToUnit(row, opts = {}) {
  ...
  const listingType = normalizeListingType(_nullishCoalesce(listingTypeRaw, () => ( propertyTypeRaw)));
  ...
  const unit = {
    ...
    ownerType: _nullishCoalesce(opts.ownerType, () => ( 'owner')),
  ...
  if (garden > 0) {
    unit.amenities = ['garden', ...(_nullishCoalesce(unit.amenities, () => ( [])))];
  }
  if (imageUrl) {
    const url = String(imageUrl).trim();
    if (url) {
      unit.featuredImage = url;
      unit.images = [url, ...(_nullishCoalesce(unit.images, () => ( [])))];
    }
  }
