 ...
 */
export async function runMatchingForLead(leadId) {
  // 1. Fetch the Lead
  ...
  const candidateUnits = await listRecords(COLLECTIONS.units, {
  ...
  await updateRecord(COLLECTIONS.stakeholders, leadId, {
    aiProfiling: {
      ...(_nullishCoalesce(lead.aiProfiling, () => ( {}))),
      topMatches: matches,
    ...
    if (match.matchScore >= 90) {
      ...
      if (unit) {
        await TelegramAlertService.sendVipMatchAlert({
          ...
          roi: _optionalChain([unit, 'access', _ => _.intelligence, 'optionalAccess', _2 => _2.roi]) || '8-12%'
        });
      }
    }
  }
 ...
 */
export async function runMatchingForUnit(unitId) {
  ...
  const candidates = (_nullishCoalesce(leadRows, () => ( []))) ;

  for (const lead of candidates) {
    const matches = await scoreMatchesWithAI(lead, [unit]);
    if (matches.length > 0 && matches[0].matchScore > 70) {
      const currentMatches = _optionalChain([lead, 'access', _3 => _3.aiProfiling, 'optionalAccess', _4 => _4.topMatches]) || [];
 ...
 */
async function scoreMatchesWithAI(lead, units) {
...
[{"unitId": "...", "matchScore": 0-100, "matchReason": "Detailed executive summary focusing on ROI potential, Capital Appreciation, and why this asset fits the stakeholder's strategic portfolio."}]`;
  ...
  const userPayload = {
    stakeholder: {
      name: lead.name,
      interests: _optionalChain([lead, 'access', _5 => _5.aiProfiling, 'optionalAccess', _6 => _6.interests]) || [],
      ...
      neuralMemory: {
        negativeSignals: _optionalChain([lead, 'access', _7 => _7.intelligence, 'optionalAccess', _8 => _8.memory, 'optionalAccess', _9 => _9.negativeSignals]) || [],
        historicalObjections: _optionalChain([lead, 'access', _10 => _10.intelligence, 'optionalAccess', _11 => _11.objections]) || [],
        cognitiveMatrix: _optionalChain([lead, 'access', _12 => _12.intelligence, 'optionalAccess', _13 => _13.matrix]) || {}
      }
    },
    assets: units.map(u => ({
      id: u.id,
  ...
  try {
    ...
    const trendsString = globalTrends ? JSON.stringify((globalTrends ).rejectionStats || globalTrends) : 'No significant trends recorded yet.';
 ...
 */
function fallbackScoring(lead, units) {
  return units.map(u => {
    let score = 50; // Base score
    
    if (lead.preferredPropertyType === u.propertyType) score += 20;
    if (lead.budgetMax && u.price <= lead.budgetMax) score += 15;
    if (_optionalChain([lead, 'access', _14 => _14.preferredLocations, 'optionalAccess', _15 => _15.includes, 'call', _16 => _16(u.compound || u.location || '')])) score += 15;
 ...
 */
export async function getMatchSummaryForLead(leadId) {
  const lead = await getRecord(COLLECTIONS.stakeholders, leadId);
  if (!lead) return "Stakeholder profile not found.";

  if (!_optionalChain([lead, 'access', _17 => _17.aiProfiling, 'optionalAccess', _18 => _18.topMatches]) || lead.aiProfiling.topMatches.length === 0) {
    return "No strategic matches detected. Initialize Neural Matching Engine.";
  }

  let summary = `<b>Matches for ${_nullishCoalesce(lead.fullName, () => ( lead.name))}:</b>\n`;
