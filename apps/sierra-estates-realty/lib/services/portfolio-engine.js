 ...
 */
export async function curateConciergePortfolio(leadId) {
  ...
  for (const match of selectedMatches) {
    ...
    conciergeUnits.push({
      id: _nullishCoalesce(unit.id, () => ( match.unitId)),
      title: unit.title,
      price: unit.price,
      matchScore: match.matchScore,
      estimatedYield: financials.annualYield,
      estimatedROI: financials.projectedROI,
      imageUrl: _nullishCoalesce(unit.featuredImage, () => ( undefined)),
  ...
  const portfolio = await insertRecord(COLLECTIONS.conciergeSelections, {
    leadId,
 ...
 */
async function generateLuxuryDescription(lead, unit) {
...
Compound: ${unit.compound}
Bedrooms: ${_nullishCoalesce(unit.bedrooms, () => ( 'N/A'))}
Client Budget: ${_nullishCoalesce(lead.budget, () => ( lead.budgetMax))} EGP
 ...
 */
async function generateSierraNoteFromTemplate(lead, units) {
 ...
 */
export function buildPortfolioMessage(portfolio) {
  return `
...
export async function trackPortfolioEngagement(
  ...
  await updateRecord(COLLECTIONS.conciergeSelections, portfolioId, {
    engagement: {
      ...(_nullishCoalesce(_optionalChain([portfolio, 'optionalAccess', _3 => _3.engagement]), () => ( {}))),
