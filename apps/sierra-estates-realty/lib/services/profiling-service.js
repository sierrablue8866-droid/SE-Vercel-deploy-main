 ...
 */
export async function extractProfileFromChat(history) {
  const prompt = `
 ...
 */
export function calculateSierraScore(profile) {
  let score = 0;
  ...
  const isPriorityLocation = profile.location && priorityCompounds.some(c => 
    _optionalChain([profile, 'access', _ => _.location, 'optionalAccess', _2 => _2.toLowerCase, 'call', _3 => _3(), 'access', _4 => _4.includes, 'call', _5 => _5(c.toLowerCase())])
  );
 ...
 */
export function getNextInterviewQuestion(profile) {
 ...
 */
export async function extractFeedbackAndSentiment(transcript) {
  const prompt = `
 ...
 */
export async function conductPrecisionInterview(leadId, transcript) {
...
(COLLECTIONS.stakeholders, leadId);
  ...
  await updateRecord(COLLECTIONS.stakeholders, leadId, {
    intelligence: {
      ...(_nullishCoalesce(_optionalChain([lead, 'optionalAccess', _6 => _6.intelligence]), () => ( {}))),
      profile: {
    ...
    aiProfiling: {
      ...(_nullishCoalesce(_optionalChain([lead, 'optionalAccess', _7 => _7.aiProfiling]), () => ( {}))),
      summary: profile.summary,
      isQualified: profile.isQualified,
    },
    orchestrationState: {
      ...(_nullishCoalesce(_optionalChain([lead, 'optionalAccess', _8 => _8.orchestrationState]), () => ( {}))),
