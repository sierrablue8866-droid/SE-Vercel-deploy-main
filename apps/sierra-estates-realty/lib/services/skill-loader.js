...
class SkillRegistry {
   __init() {this.skills = new Map()}

  constructor() {;SkillRegistry.prototype.__init.call(this);
   ...
   registerInternalSkills() {
    ...
    this.register({
      ...
      execute: async (args) => {
        try {
          ...
          const l = rows[0];
          return [
            `👤 Stakeholder: ${_nullishCoalesce(l.fullName, () => ( l.name))}`,
            `📱 Phone: ${l.phone || 'N/A'}`,
            `💰 Budget: ${l.budget || 'N/A'} – ${l.budgetMax || 'N/A'}`,
            `📍 Interests: ${_optionalChain([l, 'access', _ => _.aiProfiling, 'optionalAccess', _2 => _2.interests, 'optionalAccess', _3 => _3.join, 'call', _4 => _4(', ')]) || 'Not profiled'}`,
            `🔄 Stage: ${_optionalChain([l, 'access', _5 => _5.orchestrationState, 'optionalAccess', _6 => _6.stage]) || 'S1'}`,
            `🎯 Top Matches: ${_optionalChain([l, 'access', _7 => _7.aiProfiling, 'optionalAccess', _8 => _8.topMatches, 'optionalAccess', _9 => _9.length]) || 0} assets`
          ].join('\n');
        } catch (err) {
    ...
    this.register({
      ...
      execute: async (args) => {
        try {
          ...
          const units = await listRecords(COLLECTIONS.units, { where, limit: 5 });
          ...
          for (const u of units) {
            result += `🏠 ${u.title || u.code} — EGP ${_optionalChain([u, 'access', _10 => _10.price, 'optionalAccess', _11 => _11.toLocaleString, 'call', _12 => _12()])}\n📍 ${u.location || 'N/A'} | ${u.type || 'N/A'}\n\n`;
          }
          return result.trim();
        } catch (err) {
    ...
    this.register({
      ...
      execute: async (args) => {
        try {
          ...
          const existing = await getRecord(COLLECTIONS.stakeholders, args.leadId);
          if (!existing) return `Lead ${args.leadId} not found.`;
          await updateRecord(COLLECTIONS.stakeholders, args.leadId, {
            orchestrationState: {
              ...(_nullishCoalesce(existing.orchestrationState, () => ( {}))),
              stage: args.stage,
              status: 'active',
            },
          });
    ...
    this.register({
      ...
      execute: async (args) => {
        try {
          const u = await getRecord(COLLECTIONS.units, args.unitId);
          ...
          return [
            ...
            `⚖️ Legal Status: ${_optionalChain([u, 'access', _13 => _13.legal, 'optionalAccess', _14 => _14.status]) || 'Under Review'}`,
            `🏆 Strategic Rating: ${_optionalChain([u, 'access', _15 => _15.intelligence, 'optionalAccess', _16 => _16.valuationScore]) ? 'AAA' : 'Pending'}`
          ].join('\n');
        } catch (err) {
    ...
    this.register({
      ...
      execute: async (args) => {
        try {
          ...
          const data = await res.json() ;
          return _optionalChain([data, 'access', _17 => _17.messages, 'optionalAccess', _18 => _18[0], 'optionalAccess', _19 => _19.id])
