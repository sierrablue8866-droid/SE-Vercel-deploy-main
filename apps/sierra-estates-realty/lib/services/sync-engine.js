 ...
 */
export function getProtectedFields(firestoreData) {
  const overrides = firestoreData.manualOverrides ;
  return overrides || [];
}
 ...
 */
export async function addToDedupeQueue(record) {
  const created = await insertRecord(COLLECTIONS.syncQueue, { ...record });
  return created.id;
}
 ...
 */
export async function getPendingDedupeItems() {
  return listRecords(COLLECTIONS.syncQueue, {
...
export async function resolveDedupeItem(
  ...
  const record = await getRecord(COLLECTIONS.syncQueue, queueId);
  ...
  if (resolution === 'matched' && firestoreDocId) {
    ...
    const existing = await getRecord(COLLECTIONS.listings, firestoreDocId);

    if (existing) {
      const protectedFields = getProtectedFields(existing);
      const { merged } = mergeWithProtection(record.pfData , existing, protectedFields);
 ...
 */
export function normalizePFData(pfRaw) {
  const normalized = {
    ...
    images: Array.isArray(pfRaw.images) ? pfRaw.images.map((img) => typeof img === 'string' ? img : img.url) : [],
    pfId: pfRaw.id || '',
...
export async function syncBatch(
  ...
  for (const pfRaw of pfRawListings) {
    ...
    try {
      ...
      const exactMatches = await listRecords(
      ...
      if (exactMatches.length > 0) {
        ...
        if (conflicts.length > 0) {
          ...
          await updateRecord(COLLECTIONS.listings, fsData.id, merged );
          result.matched++;
        }
        continue;
      }
      ...
      const allListings = await listRecords(COLLECTIONS.listings);
      let bestMatch = null;
      ...
      if (bestMatch && bestMatch.confidence >= MATCH_THRESHOLD_HIGH) {
        ...
        await updateRecord(COLLECTIONS.listings, bestMatch.docId, merged );
        result.matched++;
