...
(function (D) {
  ...
  function rng(seed) { let x = seed || 1; return function () { x = (x * 1103515245 + 12345) >>> 0; return (x >>> 8) / 16777216; }; }
  const AREAS = { 'Apartment': [110, 220], 'Duplex': [200, 320], 'Twin House': [250, 340], 'Townhouse': [220, 300], 'Penthouse': [240, 380], 'Villa': [350, 620] };
  const MULT = { 'Apartment': 0.32, 'Duplex': 0.5, 'Twin House': 0.62, 'Townhouse': 0.55, 'Penthouse': 0.75, 'Villa': 1 };

  D.unitsFor = function (name) {
    if (cache[name]) return cache[name];
    const c = D.compounds.find(function (x) { return x.n === name; });
    if (!c) return [];
    const r = rng(hash(name));
    const count = 8 + Math.floor(r() * 17); // 8–24 units
    const abbr = name.replace(/\(.*\)/, '').trim().split(/\s+/).map(function (w) { return w[0]; }).join('').toUpperCase().slice(0, 3);
    const units = [];
