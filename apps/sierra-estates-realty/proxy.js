...
export async function proxy(request) {
  try {
    ...
    const requestHost = _nullishCoalesce(request.headers.get('host'), () => ( request.nextUrl.hostname));
    ...
    if (pathname.startsWith('/api/')) {
      ...
      if (pathname.startsWith('/api/internal/')) {
        ...
        const sessionToken = _optionalChain([request, 'access', _ => _.cookies, 'access', _2 => _2.get, 'call', _3 => _3(SESSION_COOKIE), 'optionalAccess', _4 => _4.value]);
        ...
        try {
          ...
          hasAdminSession =
            _optionalChain([internalSession, 'optionalAccess', _5 => _5.role]) === 'admin' || _optionalChain([internalSession, 'optionalAccess', _6 => _6.role]) === 'superadmin';
        } catch (e) {
    ...
    console.error('[Proxy Boundary Exception]:', error);
    return NextResponse.next();
  }
}
