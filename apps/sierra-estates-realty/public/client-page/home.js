...
(function () {
  ...
  if (window.SIERRA_DB && window.SIERRA_DB.isReady()) {
    Promise.all([
      window.SIERRA_DB.getListings(),
      window.SIERRA_DB.getCompounds()
    ]).then(function(results) {
      if (results[0] && results[0].length) window.HZDATA.listings = results[0];
      if (results[1] && results[1].length) window.HZDATA.compounds = results[1];
