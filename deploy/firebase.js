    ...
    onProperties: function (cb) {
      ...
      return db.collection('houyez_listings').orderBy('ai', 'desc').limit(100)
        .onSnapshot(function (snap) {
          ...
          if (items.length && window.HZDATA) {
            window.HZDATA.listings = items;
            window.dispatchEvent(new CustomEvent('sierra-data-updated', { detail: { type: 'listings', data: items } }));
          }
          if (cb) cb(items);
        });
    },
    onCompounds: function (cb) {
      ...
      return db.collection('compounds').orderBy('ai', 'desc').limit(50)
        .onSnapshot(function (snap) {
          ...
          snap.forEach(function (doc) { items.push(Object.assign({ id: doc.id }, doc.data())); });
          if (items.length && window.HZDATA) {
            window.HZDATA.compounds = items;
            window.dispatchEvent(new CustomEvent('sierra-data-updated', { detail: { type: 'compounds', data: items } }));
          }
          if (cb) cb(items);
        });
    },
