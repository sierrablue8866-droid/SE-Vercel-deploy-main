...
function Hero(props) {
  ...
  return (
    <section id="s-hero" style={{ position: 'relative', background: '#00131f', minHeight: 380, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', padding: '18px 0 0', overflow: 'hidden' }}>
      ...
      <img src="../../assets/logo-gold.png" alt="Sierra Estates" style={{ position: 'absolute', top: 6, left: 20, width: 88, height: 88, objectFit: 'contain', opacity: .22, pointerEvents: 'none', zIndex: 0 }} />
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingBottom: 18, position: 'relative' }}>
        <button onClick={function (e) {
          var el = document.getElementById('s-map');var sc = document.getElementById('root-scroll');if (el && sc) sc.scrollTo({ top: el.offsetTop - 60, behavior: 'smooth' });
          ...
          <span style={{ background: 'linear-gradient(90deg,rgba(200,150,26,.75) 0%,' + G + ' 55%,' + GL + ' 100%)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>View Map</span>
        </button>
        <div className="text" style={{ position: 'absolute', left: '50%', top: '100%', transform: 'translateX(-50%)', marginTop: 6, textAlign: 'center', pointerEvents: 'none' }}>
          <h4 className="braces" style={{ margin: 0, fontFamily: "'JetBrains Mono',monospace", fontSize: 8, fontWeight: 600, letterSpacing: '.08em', color: 'rgba(200,150,26,.55)', whiteSpace: 'nowrap' }}>click for particles</h4>
        </div>
      </div>
...
function emitParticles(x, y) {
  var host = document.getElementById('emitter');if (!host) return;
  var colors = [G, GL, '#ffffff'];
  for (var i = 0; i < 18; i++) {
    var p = document.createElement('span');
    var ang = Math.random() * Math.PI * 2;
    var dist = 40 + Math.random() * 90;
    var dx = Math.cos(ang) * dist;var dy = Math.sin(ang) * dist;
    ...
    host.appendChild(p);
    requestAnimationFrame(function (el, ddx, ddy) {return function () {
...
function Listings(props) {
  var purpose = props.purpose,search = props.search,beds = props.beds,onTap = props.onTap,saved = props.saved,onSave = props.onSave;
  var c = useApp();var dark = c.dark,lang = c.lang;
  var t = LANG[lang];var isAr = lang === 'ar';
  var C = th(dark);
...
function MapSec(props) {
  var onCpdTap = props.onCpdTap,onFilters = props.onFilters,purpose = props.purpose,onOffer = props.onOffer;
  var c = useApp();var dark = c.dark,lang = c.lang;
  var t = LANG[lang];var isAr = lang === 'ar';
  var C = th(dark);
  ...
  useEffect(function () {
    ...
    LL.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png', { maxZoom: 19 }).addTo(map);
  ...
  useEffect(function () {
    ...
    sel.forEach(function (name, si) {
      ...
      if (si === 0) {
        ...
        icon = LL.divIcon({ className: '', html: mHtml, iconSize: [40, 40], iconAnchor: [20, 40] });
      } else {
        mHtml = '<div class="s-marker"><span class="s-dot"></span><span class="s-line"></span><span class="s-lbl">' + cpd.n + ' · ' + cnt + ' · ' + pr + '</span></div>';
        icon = LL.divIcon({ className: '', html: mHtml, iconSize: [180, 16], iconAnchor: [6, 8] });
      }
    ...
    <section id="s-map" style={{ background: C.bg, transition: 'background .3s', paddingBottom: 24 }}>
      <div ref={ref} className="sec-inner" style={{ opacity: vis ? 1 : 0, transform: vis ? 'none' : 'translateY(20px)', transition: 'all .55s cubic-bezier(.16,1,.3,1)' }}>
        ...
        <div style={{ margin: '0 16px', borderRadius: 18, overflow: 'hidden', border: '1.5px solid ' + (dark ? 'rgba(200,150,26,.28)' : 'rgba(13,32,53,.12)'), boxShadow: '0 10px 36px rgba(13,32,53,.22)', position: 'relative' }}>
          <div ref={mapRef} style={{ height: 340, width: '100%', background: '#F4EFE6' }} />
          <div style={{ position: 'absolute', top: 10, left: 10, zIndex: 400, background: '#141414', border: 'none', borderRadius: 2, padding: '6px 11px', display: 'flex', alignItems: 'center', gap: 7, pointerEvents: 'none' }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#4ade80', display: 'block', animation: 'blink 2s ease infinite' }} />
            <span style={{ fontFamily: "'Plus Jakarta Sans','Inter',sans-serif", fontSize: 8, letterSpacing: '.12em', color: '#fff', fontWeight: 700, textTransform: 'uppercase' }}>{sel.length} {isAr ? 'كمبوند' : 'COMPOUNDS'} · {isAr ? 'مباشر' : 'LIVE'}</span>
          </div>
          <button onClick={onOffer} style={{ position: 'absolute', top: 10, right: 10, zIndex: 400, background: '#141414', color: '#fff', border: '1.5px solid #fff', borderRadius: 3, padding: '9px 18px', fontFamily: "'Plus Jakarta Sans','Inter',sans-serif", fontSize: 11, fontWeight: 800, letterSpacing: '.06em', textTransform: 'uppercase', cursor: 'pointer', boxShadow: '0 4px 14px rgba(0,0,0,.3)' }}>{isAr ? 'احصل على عرض' : 'GET OFFER'}</button>
        </div>
...
function Ticker() {
  ...
  var isAr = lang === 'ar';
  var items = isAr ?
  ['ماونتن فيو آي سيتي +24%', 'أب تاون كايرو +31%', 'ميفيدا إيجارات من $1,700/شهر', 'هايد بارك تقييم AI 9.8', 'الرحاب عائد إيجاري 8.1%', 'مدينتي طلب متزايد'] :
  ['Mountain View iCity +24%', 'Uptown Cairo +31%', 'Mivida rentals from $1,700/mo', 'Hyde Park AI score 9.8', 'Al Rehab rental yield 8.1%', 'Madinaty demand rising'];
  var row = items.concat(items);
  return (
    <div style={{ background: dark ? '#050E18' : '#141414', borderTop: '1px solid ' + (dark ? 'rgba(200,150,26,.2)' : 'transparent'), borderBottom: '1px solid ' + (dark ? 'rgba(200,150,26,.2)' : 'transparent'), overflow: 'hidden', padding: '9px 0' }}>
      <div style={{ display: 'flex', gap: 0, width: 'max-content', animation: 'tickerMove 26s linear infinite' }}>
        {row.map(function (s, i) {return (
            <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '0 18px', fontFamily: "'JetBrains Mono',monospace", fontSize: 9, letterSpacing: '.1em', textTransform: 'uppercase', color: GL, whiteSpace: 'nowrap' }}>
            {s}
            <span style={{ width: 4, height: 4, borderRadius: '50%', background: G, display: 'inline-block', opacity: .7 }} />
          </span>);
        })}
      </div>
    </div>);
    ...
    <section id="s-why" style={{ background: bg, padding: '0 0 32px', transition: 'background .3s' }}>
      <div ref={ref} className="sec-inner" style={{ opacity: vis ? 1 : 0, transform: vis ? 'none' : 'translateY(24px)', transition: 'all .6s cubic-bezier(.16,1,.3,1)' }}>
        ...
        <div style={{ padding: '36px 20px 0', textAlign: isAr ? 'right' : 'left', direction: isAr ? 'rtl' : 'ltr' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8, opacity: vis ? 1 : 0, transform: vis ? 'scale(1) rotate(0deg)' : 'scale(.5) rotate(-20deg)', transformOrigin: '50% 50%', transition: 'all .7s cubic-bezier(.16,1,.3,1)' }}>
            <img src="../../assets/logo-gold.png" alt="Sierra Estates" style={{ width: 34, height: 34, objectFit: 'contain', borderRadius: 8, background: '#07121E', flexShrink: 0 }} />
            <svg width="40" height="40" viewBox="-1 -1 103 103" fill="none" strokeWidth="2.2" style={{ display: 'block' }}>
              <defs>
                <linearGradient id="grad-why" x1="0" y1="0" x2="100" y2="100" gradientUnits="userSpaceOnUse">
                  <stop offset="0.2" stopColor="rgb(255,135,9)" />
                  <stop offset="0.8" stopColor="rgb(247,189,248)" />
                </linearGradient>
              </defs>
              <path stroke="url(#grad-why)" d="M50.5 50.5h50v50s-19.2 1.3-37.2-16.7S56 35.4 35.5 15.5C18.5-1 .5.5.5.5v50h50s25.6-.6 38-18 12-32 12-32h-50v100H.5S.2 80.7 11.8 68.2 40 49.7 50.5 50.5Z" />
            </svg>
          </div>
          <div style={{ fontFamily: isAr ? "'Cairo',sans-serif" : "'Cormorant Garamond',serif", fontSize: isAr ? 34 : 42, fontWeight: isAr ? 700 : 400, lineHeight: 1.0, color: dark ? '#fff' : '#0D2035', marginBottom: 6 }}>
            {isAr ? 'أبعد من الوساطة' : ['Beyond', 'Brokerage'].map(function (word, wi) {return React.createElement(React.Fragment, { key: wi },
              wi > 0 ? React.createElement('br', null) : null,
              React.createElement('span', { style: { display: 'inline-block', opacity: vis ? 1 : 0, transform: vis ? 'translateY(0)' : 'translateY(16px)', transition: 'all .5s cubic-bezier(.16,1,.3,1) ' + (wi * .12) + 's' } }, word)
            );})}
          </div>
...
Object.assign(window, { Hero, Listings, MapSec, TourSec, AIHub, WhySec, AboutUs, ContactUs, Ticker });
