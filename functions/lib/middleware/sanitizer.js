...
exports.INJECTION_TEST_CASES = void 0;
exports.sanitizeAgentInput = sanitizeAgentInput;
 ...
 */
function sanitizeAgentInput(input, maxLength = 2000) {
    // 1. Check length
    ...
    for (const pattern of INJECTION_PATTERNS) {
        if (pattern.test(sanitized)) {
            firebase_functions_1.logger.warn(`[SECURITY] Potential injection attempt detected:`, {
                pattern: pattern.source,
                input: sanitized.substring(0, 100),
            });
            throw new Error("Invalid input detected. Please avoid system commands or instructions.");
        }
    }
    // 4. Remove null bytes
    ...
    firebase_functions_1.logger.info(`[AUDIT] Input sanitized successfully`, {
...
function validateJsonPayload(payload) {
    ...
    for (const dangerous of dangerousKeys) {
        if (keys.some((k) => k.includes(dangerous))) {
            throw new Error(`Payload contains forbidden key: ${dangerous}`);
        }
    }
}
/**
 ...
 */
async function checkRateLimit(userId, limit = 100, // requests per window
windowMs = 60000 // 1 minute window
    ...
    try {
        ...
        // Increment count
        if (data.count >= limit) {
            firebase_functions_1.logger.warn(`[SECURITY] Rate limit exceeded for user`, { userId });
            return false;
        }
        ...
        firebase_functions_1.logger.error(`Rate limit check failed:`, error);
...
function sanitizationMiddleware(req, res, next) {
    try {
        ...
        if (req.body && typeof req.body === "object") {
            for (const [key, value] of Object.entries(req.body)) {
                if (typeof value === "string") {
                    req.body[key] = sanitizeAgentInput(value);
                }
            }
        }
        ...
        for (const [key, value] of Object.entries(req.query)) {
            if (typeof value === "string") {
                req.query[key] = sanitizeAgentInput(value);
            }
        }
        next();
    }
    catch (error) {
        firebase_functions_1.logger.error(`[SECURITY] Sanitization failed:`, error);
        res.status(400).json({
