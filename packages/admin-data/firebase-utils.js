...
function envVar(name) {
  ...
  return _nullishCoalesce(env[`NEXT_PUBLIC_${name}`], () => ( env[`VITE_${name}`]));
}
...
export async function fetchListings(opts
  ...
  if (_optionalChain([opts, 'optionalAccess', _ => _.status])) constraints.push(where('status', '==', opts.status));
  if (_optionalChain([opts, 'optionalAccess', _2 => _2.mode])) constraints.push(where('mode', '==', opts.mode));
  if (_optionalChain([opts, 'optionalAccess', _3 => _3.compound])) constraints.push(where('compound_name', '==', opts.compound));
  constraints.push(orderBy('created_at', 'desc'));
  if (_optionalChain([opts, 'optionalAccess', _4 => _4.limitCount])) constraints.push(limit(opts.limitCount));
 ...
 */
export async function deleteListingAndOwner(listingId) {
  const db = getDb();
  const batch = writeBatch(db);
 ...
 */
export async function fetchAllOwners(limitCount = 100) {
  const db = getDb();
  const q = query(
...
export async function fetchRequests(opts
  ...
  if (_optionalChain([opts, 'optionalAccess', _5 => _5.status])) constraints.push(where('status', '==', opts.status));
  if (_optionalChain([opts, 'optionalAccess', _6 => _6.assignedAgentId]))
    constraints.push(where('assigned_agent_id', '==', opts.assignedAgentId));
  if (_optionalChain([opts, 'optionalAccess', _7 => _7.clientId]))
    constraints.push(where('client_id', '==', opts.clientId));
  constraints.push(orderBy('created_at', 'desc'));
  if (_optionalChain([opts, 'optionalAccess', _8 => _8.limitCount])) constraints.push(limit(opts.limitCount));
...
export async function fetchClients(limitCount = 100) {
  const db = getDb();
  const q = query(
