 function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }import { describe, it, expect, vi, beforeEach } from 'vitest';
...
describe('packages/agents-core', () => {
  describe('AgentRegistry', () => {
    ...
    it('should retrieve a known agent profile by name', () => {
      ...
      expect(_optionalChain([agent, 'optionalAccess', _ => _.name])).toBe('backend-specialist');
      expect(_optionalChain([agent, 'optionalAccess', _2 => _2.systemPrompt])).toBeDefined();
      expect(_optionalChain([agent, 'optionalAccess', _3 => _3.systemPrompt, 'access', _4 => _4.length])).toBeGreaterThan(0);
    });
  ...
  describe('AgentOrchestrator', () => {
    ...
    beforeEach(() => {
      mockCompletion = vi.fn(async (agentName, stage) => {
