...
export interface NegotiationOffer {
  round: number;
  proposedBy: 'buyer' | 'seller' | 'closer';
  price: number;
...
export interface NegotiationOutcome {
  status: 'agreement_reached' | 'counter_offered' | 'deadlock';
  finalPrice?: number;
  totalDiscountPercent: number;
  agreedTenureYears?: number;
  commissionFeeEGP: number;
  rounds: NegotiationOffer[];
