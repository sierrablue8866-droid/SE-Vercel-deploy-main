...
export interface VoiceNoteInput {
  audioUrl: string;
  mimeType: 'audio/ogg' | 'audio/m4a' | 'audio/mp3' | 'audio/wav';
  durationSeconds: number;
  spokenTranscriptAr: string;
  brokerPhone: string;
}
...
export interface ExtractedVoiceListing {
  ...
  extractedPriceEGP?: number;
  extractedDownPaymentPercent?: number;
  extractedTenureYears?: number;
...
export class VoiceTranscriberEngine {
   ...
   */
  private static normalizeDigits(str: string): string {
   ...
   */
  public static parseSpokenTranscript(input: VoiceNoteInput): ExtractedVoiceListing {
    const text = this.normalizeDigits(input.spokenTranscriptAr);
    let confidence = 70;

    // Detect Compound
    let detectedCompound: string | undefined;
    ...
    // Detect Unit Type
    let detectedUnitType: string | undefined;
    ...
    let extractedPriceEGP: number | undefined;
    ...
    // Detect Down Payment %
    let extractedDownPaymentPercent: number | undefined;
    ...
    // Detect Tenure Years
    let extractedTenureYears: number | undefined;
