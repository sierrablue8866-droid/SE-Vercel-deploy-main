 function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } }// sierra-estatese/config/views.ts
...
export function getViewsByRole(role) {
  const hierarchy = {
  ...
  const allowed = _nullishCoalesce(hierarchy[role], () => ( ["public"]));
