 function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } } function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }import * as vscode from 'vscode';
...
export function activate(context) {
    ...
    is_enabled = _nullishCoalesce(config.get('enabled'), () => ( true));
...
function update_status_bar(state) {
    const build = 'B';
...
function getUserId(context, config) {
    // 1. Check if user has configured a custom userId
    const configuredUserId = config.get('userId');
    if (configuredUserId) return configuredUserId;

    // 2. Check if we have a persistent userId in global state
    let persistedUserId = context.globalState.get('openmemory.userId');
...
async function start_session() {
    try {
        const config = vscode.workspace.getConfiguration('openmemory');
        const configuredProject = config.get('projectName');
        const project = configuredProject || _optionalChain([vscode, 'access', _ => _.workspace, 'access', _2 => _2.workspaceFolders, 'optionalAccess', _3 => _3[0], 'optionalAccess', _4 => _4.name]) || 'unknown';
...
async function query_context(query, file) {
...
async function add_memory(content, file) {
