...
class TextProcessor {
     ...
     */
    static tokenizeWithPosition(text) {
    ...
    static extractEntities(text) {
        ...
        const quoted = _optionalChain([text, 'access', _ => _.match, 'call', _2 => _2(/"([^"]+)"/g), 'optionalAccess', _3 => _3.map, 'call', _4 => _4(q => q.replace(/"/g, ''))]) || []
        const patterns = [
            /\b[A-Z]{2,}\b/g,
     ...
     */
    static extractMostInformativeSentences(text, count = 3) {
        const sentences = this.sentenceSplit(text)
     ...
     */
    static detectDiscourseMarkers(text) {
     ...
     */
    static computeBM25(query, document, k1 = 1.5, b = 0.75, avgDocLength = 100) {
...
class QueryAnalyzer {
     ...
     */
    static analyzeTemporalContext(query) {
        const lower = query.toLowerCase()
    ...
    static computeRelevanceScore(
        ...
        const sectorAffinity = _optionalChain([SECTOR_AFFINITIES, 'access', _5 => _5[memory.sector], 'optionalAccess', _6 => _6.includes, 'call', _7 => _7(context.intent)]) ? 1.2 : 1.0
    ...
    static computeTemporalRelevance(
        ...
        if (!_optionalChain([context, 'access', _8 => _8.temporalContext, 'optionalAccess', _9 => _9.hasTimeReference]) || !memory.last_seen_at) {
            return 1.0
        }
...
class ContextWindowManager {
     static  __initStatic() {this.MAX_CONTEXT_TOKENS = 8000}
     static  __initStatic2() {this.ATTENTION_DECAY = 0.95}
    ...
    static buildContextWindow(
        ...
        const temporalRelevance = new Map()
...
} ContextWindowManager.__initStatic(); ContextWindowManager.__initStatic2();
...
class SemanticGraphBuilder {
     ...
     */
     static detectRelationType(textA, textB) {
     ...
     */
    static findCentralNodes(graph, topN = 5) {
...
class MemoryClusterer {
    ...
    static cluster(
        ...
        for (let i = 0; i < Math.min(maxClusters, memories.length); i++) {
            ...
            const clusterMembers = [memories[seedIdx]]
     ...
     */
    static selectRepresentative(cluster) {
     ...
     */
    static mergeSimilarClusters(clusters, threshold = 0.6) {
...
class CoherenceScorer {
     ...
     */
    static scoreTextCoherence(text) {
     ...
     */
    static scoreSegmentCoherence(segments) {
     ...
     */
    static scoreCompleteness(answer, query, memories) {
...
class AnswerSynthesizer {
     ...
     */
     static synthesizeInterrogativeAnswer(
        clusters,
        rankedMemories,
        relevanceMap,
        attentionScores,
     ...
     */
     static synthesizeDeclarativeAnswer(
        clusters,
        rankedMemories,
        relevanceMap,
        attentionScores,
     ...
     */
     static calculateDiversity(segments) {
     ...
     */
     static calculateLengthVariance(segments) {
...
export class MemoryAIEngine {
     ...
     */
    static clusterMemories(memories, maxClusters = 5) {
     ...
     * Build semantic graph from memories
     */
    static buildSemanticGraph(memories) {
     ...
     */
    static findCentralMemories(memories, topN = 5) {
 ...
 = {}
     ...
     */
    static generateQueryAlternatives(query) {
        const context = QueryAnalyzer.analyze(query)
        const alternatives = []
...

 {
        ...
        const dominantSector = _optionalChain([sectors, 'access', _10 => _10.sort, 'call', _11 => _11((a, b) => b[1] - a[1]), 'access', _12 => _12[0], 'optionalAccess', _13 => _13[0]]) || 'semantic'
...

 {
        ...
        if (_optionalChain([context, 'access', _14 => _14.temporalContext, 'optionalAccess', _15 => _15.hasTimeReference])) {
            strategy = 'temporal'
        ...
        for (const memory of memories) {
            ...
            if (similar.length > 0) {
                ...
                merged.push({
                    ...
                    last_seen_at: _optionalChain([mostRecent, 'optionalAccess', _16 => _16.last_seen_at]) || memory.last_seen_at
                })
