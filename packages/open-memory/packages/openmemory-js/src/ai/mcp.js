...
const uid = (val) => (_optionalChain([val, 'optionalAccess', _ => _.trim, 'call', _2 => _2()]) ? val.trim() : undefined);
...
export const create_mcp_srv = (tenant) => {
    ...
    registry.tool(
        ...
        async ({
            ...
            if (type === "contextual" || type === "unified") {
                ...
                const matches = await hsg_query(query, _nullishCoalesce(k, () => ( 8)), flt);
                results.contextual = matches.map((m) => ({
                    source: "hsg",
                    id: m.id,
            ...
            if (type === "factual" || type === "unified") {
                const facts = await query_facts_at_time({
                    user_id: _nullishCoalesce(u, () => ( "anonymous")),
                    project_id: proj,
                    subject: _optionalChain([fact_pattern, 'optionalAccess', _3 => _3.subject]),
                    predicate: _optionalChain([fact_pattern, 'optionalAccess', _4 => _4.predicate]),
                    object: _optionalChain([fact_pattern, 'optionalAccess', _5 => _5.object]),
                    at: at_date,
                    min_confidence: 0.0,
                });

                results.factual = facts.map((f) => ({
                    source: "temporal",
                    id: f.id,
                    subject: f.subject,
            ...
            if (type === "contextual") {
                ...
                const ctx_count = _optionalChain([results, 'access', _6 => _6.contextual, 'optionalAccess', _7 => _7.length]) || 0;
                const fact_count = _optionalChain([results, 'access', _8 => _8.factual, 'optionalAccess', _9 => _9.length]) || 0;
            ...
            if ((type === "factual" || type === "both") && facts) {
                ...
                for (const fact of facts) {
                    ...
                    const fact_id = await insert_fact({
                        ...
                        object: fact.object,
                        valid_from,
                        confidence: _nullishCoalesce(fact.confidence, () => ( 1.0)),
                        metadata,
                        user_id: u,
                        project_id: proj,
                    });
                ...
                content: [
                    { type: "text", text: txt },
                    {
                        ...
                        text: JSON.stringify(
                            {
                                ...results,
                                user_id: _nullishCoalesce(u, () => ( null)),
                                project_id: proj,
                            },
                            null,
                            2,
            ...
            if (proj) {
                ...
                `,
                    [
                        ...
                        ...(u ? [u] : []),
                        ...(sector ? [sector] : []),
                        _nullishCoalesce(limit, () => ( 10)),
                    ],
                );
            } else if (u) {
                const all = await q.all_mem_by_user.all(u, _nullishCoalesce(limit, () => ( 10)), 0);
                rows = sector
                    ? all.filter((row) => row.primary_sector === sector)
                    : all;
            } else {
                rows = sector
                    ? await q.all_mem_by_sector.all(sector, _nullishCoalesce(limit, () => ( 10)), 0)
                    : await q.all_mem.all(_nullishCoalesce(limit, () => ( 10)), 0);
            }
        ...
        try {
            ...
            if (!res.headersSent)
                send_err(
                    ...
                    _nullishCoalesce(_optionalChain([(error ), 'optionalAccess', _10 => _10.id]), () => ( null)),
                    500,
                );
        }
    };
