 function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } }import path from "path";
...
,
    ...
    lg_reflective: (_nullishCoalesce(process.env.OM_LG_REFLECTIVE, () => ( "true"))) !== "false",
    ...
    use_summary_only: (_nullishCoalesce(process.env.OM_USE_SUMMARY_ONLY, () => ( "true"))) !== "false",
    ...
    decay_reinforce_on_query:
        (_nullishCoalesce(process.env.OM_DECAY_REINFORCE_ON_QUERY, () => ( "true"))) !== "false",
    regeneration_enabled:
        (_nullishCoalesce(process.env.OM_REGENERATION_ENABLED, () => ( "true"))) !== "false",
