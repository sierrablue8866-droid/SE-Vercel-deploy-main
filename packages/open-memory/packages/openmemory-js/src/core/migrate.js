 function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }import { env } from "./cfg";
        ...
        db.get(
            ...
            (err, row) => {
                ...
                db.get(
                    ...
                    (e, v) => {
                        if (e) return no(e);
                        ok(_optionalChain([v, 'optionalAccess', _ => _.version]) || null);
                    },
                );
            },
        );
 ...
 */
async function resolveSqliteVectorTable(db) {
...
async function get_db_version_pg(pool) {
    try {
        ...
        return _optionalChain([ver, 'access', _2 => _2.rows, 'access', _3 => _3[0], 'optionalAccess', _4 => _4.version]) || null;
    } catch (e) {
        return null;
    }
}
...
async function quarantine_orphan_temporal_facts_sqlite(
    ...
    const tableExists = await new Promise((ok, no) => {
        db.get(
...
async function quarantine_orphan_temporal_facts_pg(pool) {
    const sc = pgSchema();
    const check = await pool.query(
        `SELECT EXISTS (
            SELECT FROM information_schema.tables
