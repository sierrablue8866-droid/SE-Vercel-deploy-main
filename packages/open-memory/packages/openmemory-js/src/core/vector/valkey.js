...
export class ValkeyVectorStore  {
 ...
 = [];
            do {
                ...
                if (keys.length) {
                    ...
                    _optionalChain([buffers, 'optionalAccess', _ => _.forEach, 'call', _2 => _2((b, idx) => {
                        if (b && b[1]) {
                            const [buf, vec_user_id, vec_project_id] = b[1] 
 ...
 = [];
        ...
        do {
            ...
            if (keys.length) {
                ...
                _optionalChain([res, 'optionalAccess', _3 => _3.forEach, 'call', _4 => _4((r, idx) => {
                    if (r && r[1]) {
                        const [v, dim] = r[1] ;
                        const key = keys[idx];
    ...
    async getVectorsBySector(
        ...
        do {
            ...
            if (keys.length) {
                ...
                keys.forEach((k) => pipe.hmget(k, "v", "dim"));
                const res = await pipe.exec();
                _optionalChain([res, 'optionalAccess', _5 => _5.forEach, 'call', _6 => _6((r, idx) => {
                    if (r && r[1]) {
                        const [v, dim] = r[1] ;
                        const key = keys[idx];
