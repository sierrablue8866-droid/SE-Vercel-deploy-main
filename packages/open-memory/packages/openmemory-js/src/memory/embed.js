 function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } }import { env, tier } from "../core/cfg";
...
export async function embedForSector(t, s) {
    console.error(
...
async function emb_ollama(t, s) {
    ...
    return resize_vec(((await r.json()) ).embedding, env.vec_dim);
}
async function emb_aws(t, s) {
    ...
    const dim = _nullishCoalesce([256, 512, 1024].find((x) => x >= env.vec_dim), () => ( 1024));
    const params = {
        modelId: m,
...
export function gen_syn_emb(t, s) {
