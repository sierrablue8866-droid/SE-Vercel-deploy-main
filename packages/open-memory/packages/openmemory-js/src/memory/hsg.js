...
export const sector_relationships = {
    semantic: {
        procedural: 0.8,
        episodic: 0.6,
        reflective: 0.7,
...
async function compute_tag_match_score(
    ...
    if (!_optionalChain([mem, 'optionalAccess', _ => _.tags])) return 0;
...
export function classify_content(
    ...
    if (_optionalChain([metadata, 'optionalAccess', _2 => _2.sector]) && sectors.includes(metadata.sector)) {
        return {
    ...
    const confidence =
        primaryScore > 0
            ? Math.min(
                  1.0,
                  primaryScore /
                      (primaryScore + (_optionalChain([sortedScores, 'access', _3 => _3[1], 'optionalAccess', _4 => _4[1]]) || 0) + 1),
              )
            : 0.2;
    return {
...
export function calc_recency_score(last_seen) {
    const now = Date.now();
...
export function boosted_sim(s) {
    return 1 - Math.exp(-hybrid_params.tau * s);
}
export function compute_simhash(text) {
...
export function hamming_dist(hash1, hash2) {
    let dist = 0;
...
export function calc_mean_vec(
    ...
    const sec_scores = emb_res.map((r) => ({
        vector: r.vector,
        confidence: _optionalChain([sector_configs, 'access', _5 => _5[r.sector], 'optionalAccess', _6 => _6.weight]) || 1.0,
    }));
    ...
    for (const result of emb_res) {
        const sec_wt = _optionalChain([sector_configs, 'access', _7 => _7[result.sector], 'optionalAccess', _8 => _8.weight]) || 1.0;
...
export async function reinforce_waypoints(trav_path) {
    const now = Date.now();
...
export async function prune_weak_waypoints() {
...
setInterval(async () => {
    ...
    for (const [a, b] of pairs) {
        try {
            ...
            const cur_wt = _optionalChain([wp, 'optionalAccess', _9 => _9.weight]) || 0;
            const new_wt = Math.min(
                1,
                cur_wt + hybrid_params.eta * (1 - cur_wt) * temp_fact,
            );
            const project_id =
                _optionalChain([memA, 'optionalAccess', _10 => _10.project_id]) || _optionalChain([memB, 'optionalAccess', _11 => _11.project_id]) || _optionalChain([wp, 'optionalAccess', _12 => _12.project_id]) || null;
            const uid =
                _optionalChain([memA, 'optionalAccess', _13 => _13.user_id]) || _optionalChain([memB, 'optionalAccess', _14 => _14.user_id]) || _optionalChain([wp, 'optionalAccess', _15 => _15.user_id]) || "anonymous";
            await q.ins_waypoint.run(
                a,
                b,
                uid,
                project_id,
                new_wt,
                _optionalChain([wp, 'optionalAccess', _16 => _16.created_at]) || now,
                now,
            );
        } catch (e) {}
    }
...
const get_sal = async (id, def_sal) => {
    ...
    const s = _nullishCoalesce(_optionalChain([m, 'optionalAccess', _17 => _17.salience]), () => ( def_sal));
...
,
    ...
    try {
        ...
        let ss;
        if (_optionalChain([f, 'optionalAccess', _18 => _18.sectors, 'optionalAccess', _19 => _19.length])) {
            ss = f.sectors;
        } else {
            ss = [...sectors];
        }
 ...
 = {};
        for (const s of ss) {
            ...
            const results = await vector_store.searchSimilar(
                ...
                qv,
                k * 3,
                _optionalChain([f, 'optionalAccess', _20 => _20.user_id]),
                _optionalChain([f, 'optionalAccess', _21 => _21.project_id]),
            );
...
;
        ...
        for (const mid of Array.from(ids)) {
            ...
            if (!m || (_optionalChain([f, 'optionalAccess', _22 => _22.minSalience]) && m.salience < f.minSalience)) continue;
            if (_optionalChain([f, 'optionalAccess', _23 => _23.user_id]) && m.user_id !== f.user_id) continue;

            // Project isolation check: match current project or system_global or legacy null
            if (_optionalChain([f, 'optionalAccess', _24 => _24.project_id])) {
                const isMatch =
            ...
            if (_optionalChain([f, 'optionalAccess', _25 => _25.startTime]) && m.created_at < f.startTime) continue;
            if (_optionalChain([f, 'optionalAccess', _26 => _26.endTime]) && m.created_at > f.endTime) continue;
            ...
            if (
                ...
                sector_penalty =
                    _optionalChain([sector_relationships, 'access', _27 => _27[query_sector], 'optionalAccess', _28 => _28[mem_sector]]) || 0.3;
            }
            const adjusted_sim = bs * sector_penalty;

            const em = exp.find((e) => e.id === mid);

            const ww = Math.min(1.0, Math.max(0, _optionalChain([em, 'optionalAccess', _29 => _29.weight]) || 0));
            ...
            if (_optionalChain([f, 'optionalAccess', _30 => _30.project_id]) && m.project_id === f.project_id) {
            ...
            res.push({
                ...
                path: _optionalChain([em, 'optionalAccess', _31 => _31.path]) || [mid],
                salience: sal,
        ...
        for (const r of top) {
            const cur_fb = await _asyncOptionalChain([(await q.get_mem.get(r.id)), 'optionalAccess', async _32 => _32.feedback_score]) || 0;
...
export async function run_decay_process()
...
export async function add_hsg_memory(
...

 {
    ...
    try {
        ...
        let cur_seg = _nullishCoalesce(_optionalChain([max_seg_res, 'optionalAccess', _33 => _33.max_seg]), () => ( 0));
        const seg_cnt_res = await q.get_segment_count.get(cur_seg);
        const seg_cnt = _nullishCoalesce(_optionalChain([seg_cnt_res, 'optionalAccess', _34 => _34.c]), () => ( 0));
...
export async function delete_memory(id) {
