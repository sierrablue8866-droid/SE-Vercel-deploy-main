 function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }import { q, log_maint_op } from "../core/db";
...
const cluster = (mems) => {
    ...
    for (const m of mems) {
        if (
            ...
            _optionalChain([m, 'access', _ => _.metadata, 'optionalAccess', _2 => _2.consolidated])
        )
            continue;
        const c = { mem: [m], n: 1 };
        used.add(m.id);
