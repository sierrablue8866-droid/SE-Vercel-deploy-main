 ...
 */
function read_project(req) {
    const candidates = [
        _optionalChain([req, 'optionalAccess', _ => _.project_id]),
        _optionalChain([req, 'optionalAccess', _2 => _2.headers, 'optionalAccess', _3 => _3["x-project-id"]]),
        _optionalChain([req, 'optionalAccess', _4 => _4.query, 'optionalAccess', _5 => _5.project_id]),
        _optionalChain([req, 'optionalAccess', _6 => _6.body, 'optionalAccess', _7 => _7.project_id]),
    ];
...
const update_fact_schema = {
    confidence: { type: "number", min: 0, max: 1 },
    metadata: { type: "object" },
};

const invalidate_fact_schema = {
...
export const create_temporal_fact = async (req, res) => {
...
(res, req.body, create_fact_schema);
    ...
    const valid_from_date = _nullishCoalesce(vf.date, () => ( new Date()));
    const conf =
    ...
    try {
        ...
        res.json({
            ...
            confidence: conf,
            user_id: tenant,
            project_id: _nullishCoalesce(project_id, () => ( null)),
...
export const get_temporal_fact = async (req, res) => {
    const tenant = require_tenant(req, res);
    if (!tenant) return;
    try {
        const subject =
        ...
        const at_date = _nullishCoalesce(at_parsed.date, () => ( new Date()));
...
export const get_current_temporal_fact = async (req, res) => {
    const tenant = require_tenant(req, res);
    if (!tenant) return;
    try {
        const subject =
...
export const get_entity_timeline = async (req, res) => {
    const tenant = require_tenant(req, res);
    if (!tenant) return;
    try {
        const subject =
        ...
        const timeline = timeline_raw.filter((entry) => {
            const u = _nullishCoalesce(_optionalChain([entry, 'access', _8 => _8.fact, 'optionalAccess', _9 => _9.user_id]), () => ( entry.user_id));
...
export const get_predicate_history = async (req, res) => {
    const tenant = require_tenant(req, res);
    if (!tenant) return;
    try {
        const predicate =
        ...
        const timeline = timeline_raw.filter((entry) => {
            const u = _nullishCoalesce(_optionalChain([entry, 'access', _10 => _10.fact, 'optionalAccess', _11 => _11.user_id]), () => ( entry.user_id));
            return u === undefined || u === null || u === tenant;
        });

        res.json({
            predicate,
            from: _optionalChain([from_p, 'access', _12 => _12.date, 'optionalAccess', _13 => _13.toISOString, 'call', _14 => _14()]),
            to: _optionalChain([to_p, 'access', _15 => _15.date, 'optionalAccess', _16 => _16.toISOString, 'call', _17 => _17()]),
            timeline,
...
export const update_temporal_fact = async (req, res) => {
    const tenant = require_tenant(req, res);
    if (!tenant) return;
...
export const invalidate_temporal_fact = async (req, res) => {
    const tenant = require_tenant(req, res);
    if (!tenant) return;
    const id = req.params.id;
    if (!id) return res.status(400).json({ error: "Fact ID is required" });
    ...
    const valid_to_date = _nullishCoalesce(vt.date, () => ( new Date()));
...
export const get_subject_facts = async (req, res) => {
    const tenant = require_tenant(req, res);
    if (!tenant) return;
    try {
        ...
        res.json({
            subject,
            at: _optionalChain([at_p, 'access', _18 => _18.date, 'optionalAccess', _19 => _19.toISOString, 'call', _20 => _20()]),
            include_historical: include_hist,
            facts,
            count: facts.length,
        });
...
export const search_temporal_facts = async (req, res) => {
    const tenant = require_tenant(req, res);
    if (!tenant) return;
    try {
        const pattern =
        ...
        res.json({
            pattern,
            field,
            at: _optionalChain([at_p, 'access', _21 => _21.date, 'optionalAccess', _22 => _22.toISOString, 'call', _23 => _23()]),
            facts,
            count: facts.length,
        });
    } catch (error) {
...
export const get_temporal_stats = async (req, res) => {
    const tenant = require_tenant(req, res);
    if (!tenant) return;
    try {
...
export const apply_decay = async (req, res) => {
    const tenant = require_tenant(req, res);
    if (!tenant) return;
    ...
    if (!b) return;
    const decay_rate = _nullishCoalesce(b.decay_rate, () => ( 0.01));
...
export const compare_facts = async (req, res) => {
    const tenant = require_tenant(req, res);
    if (!tenant) return;
    try {
        const subject =
...
export const get_most_volatile = async (req, res) => {
    const tenant = require_tenant(req, res);
    if (!tenant) return;
    try {
        const subject =
        ...
        const volatile = volatile_raw.filter((f) => {
            const u = _nullishCoalesce(f.user_id, () => ( _optionalChain([f, 'access', _24 => _24.fact, 'optionalAccess', _25 => _25.user_id])));
            return u === undefined || u === null || u === tenant;
        });
