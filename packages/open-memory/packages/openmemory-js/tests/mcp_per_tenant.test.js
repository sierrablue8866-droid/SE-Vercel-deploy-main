 function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } } function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }// Force synthetic embeddings + sqlite backend BEFORE importing anything
...
async function connect_client(tenant) {
...
function parse_items(result) {
    // openmemory_list returns two text blocks; the second is a JSON dump.
    const blocks = (_nullishCoalesce(_optionalChain([result, 'optionalAccess', _ => _.content]), () => ( []))) 
...
;
    ...
    return _nullishCoalesce(parsed.items, () => ( []));
}

function parse_store(result) {
    const blocks = (_nullishCoalesce(_optionalChain([result, 'optionalAccess', _2 => _2.content]), () => ( []))) 
...
;
    ...
    if (!jsonBlock) return {};
    const parsed = JSON.parse(jsonBlock);
    return { id: _optionalChain([parsed, 'optionalAccess', _3 => _3.hsg, 'optionalAccess', _4 => _4.id]), project_id: _optionalChain([parsed, 'optionalAccess', _5 => _5.project_id]) };
}
...
describe("MCP per-tenant scoping", () => {
    ...
    it("openmemory_store rejects a user_id arg that disagrees with the tenant", async () => {
        ...
        const text = (_nullishCoalesce(result.content, () => ( [])))
            .map((b) => _nullishCoalesce(b.text, () => ( "")))
            .join("\n");
