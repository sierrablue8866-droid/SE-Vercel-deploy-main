...
console.log('════════════════════════════════════════════════════════════════');
console.log('  SIERRA ESTATES: 4-SHEET SEGREGATED MASTER INVENTORY GENERATOR');
console.log('  [Owners Rent] | [Owners Buy] | [Broker Rent] | [Broker Buy]   ');
console.log('════════════════════════════════════════════════════════════════\n');
...
console.log(`📊 Initial record count: ${uncleanedRows.length}`);
...
function isOwner(r) {
  ...
  const ob = String(r.OwnerBroker || r.SourceType || '').toLowerCase();
  const desc = String(r.Comment || r.AdditionalFeatures || '').toLowerCase();
  
  if (sheet.includes('owner') || sheet.includes('مالك')) return true;
  if (ob.includes('owner') || ob.includes('direct') || ob.includes('مالك')) return true;
  if (desc.includes('من المالك') || desc.includes('مالك مباشر') || desc.includes('direct owner') || desc.includes('مع المالك')) return true;
  return false;
}
...
function isRent(r) {
  ...
  if (sheet.includes('rent') || sheet.includes('ايجار') || sheet.includes('إيجار') || sheet.includes('ايجارات')) return true;
  if (cat.includes('rent') || cat.includes('ايجار')) return true;
  if (desc.includes('للايجار') || desc.includes('ايجار') || desc.includes('شهريا') || desc.includes('شهري')) return true;
  if (price > 0 && price <= 250000 && !desc.includes('مقدم') && !desc.includes('قسط') && !sheet.includes('resale') && !sheet.includes('sale')) return true;
  return false;
}
...
// Partition rows into the 4 requested buckets
const buckets = {
  'Owners_Rent': [],
  'Owners_Buy': [],
  'Broker_Rent': [],
...
rawRows.forEach(r => {
  ...
  const rent = isRent(r);
  if (owner && rent) buckets['Owners_Rent'].push(r);
  else if (owner && !rent) buckets['Owners_Buy'].push(r);
  else if (!owner && rent) buckets['Broker_Rent'].push(r);
  else buckets['Broker_Buy'].push(r);
});

// Deduplicate and process each bucket
...
console.log('✅ Partitioning & Deduplication Results:');
...
const totalAll = processedSheets['Owners_Rent'].length + processedSheets['Owners_Buy'].length + processedSheets['Broker_Rent'].length + processedSheets['Broker_Buy'].length;
console.log(`   Total Unique Across All 4 Sheets: ${totalAll} units\n`);

// 3. Assemble 4-Sheet Excel Workbook
...
console.log(`💾 Saved 4-sheet Excel workbook → ${masterXlsxPath} (${(fs.statSync(masterXlsxPath).size / (1024 * 1024)).toFixed(2)} MB)`);
console.log(`💾 Saved 4-sheet Excel workbook → ${rootXlsxPath}`);

// 4. Also Export Individual Segment CSVs & Individual XLSX files for blazing fast downloads!
...
sheetConfig.forEach(({ key, title }) => {
  ...
  const csvFile = path.join(downloadsDir, `sierra-estates-${key.toLowerCase().replace('_', '-')}.csv`);
  fs.writeFileSync(csvFile, csvContent, 'utf8');
  console.log(`   ✓ Exported individual CSV  → ${path.basename(csvFile)} (${(fs.statSync(csvFile).size / 1024).toFixed(1)} KB)`);

  // Individual single-sheet XLSX (lightweight & instant opening!)
  const singleWb = xlsx.utils.book_new();
  xlsx.utils.book_append_sheet(singleWb, ws, title);
  const singleXlsx = path.join(downloadsDir, `sierra-estates-${key.toLowerCase().replace('_', '-')}.xlsx`);
...
// 5. Keep Full Combined Clean CSV as well
...
console.log('\n🎉 Successfully rebuilt master inventory into 4 dedicated, lightning-fast sheets!');
