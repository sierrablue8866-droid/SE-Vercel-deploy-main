...
import { classifySourceType, isNewListing, isWithinOneMonth } from '../packages/agents/tools/whatsappGroupRegistry';
import { batchIngestListings, UnitListingData } from '../packages/agents/tools/inventoryTools';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

interface ExtractedChatMessage {
  timestamp: string;
  sender: string;
  text: string;
  groupName: string;
 ...
 */
export function parseRawWhatsAppChatLog(rawContent: string, defaultGroupName = 'WhatsApp Chat Export'): ExtractedChatMessage[] {
