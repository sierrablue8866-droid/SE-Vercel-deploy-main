...
interface UnifiedInventoryUnit {
  id: string;
  sierraCode: string;
  title?: string;
  titleAr?: string;
  ...
  listedAt: string;
  description?: string;
  origin: 'master_sheet' | 'whatsapp_group' | 'obsidian_memory';
}
...
function isNew(ts: string | Date | undefined): boolean {
  if (!ts) return false;
...
function runMergeAndReport() {
  ...
  const unifiedMap = new Map<string, UnifiedInventoryUnit>();
  ...
  if (fs.existsSync(MASTER_SHEET_PATH)) {
    ...
    for (const u of rawMaster) {
      ...
      const unit: UnifiedInventoryUnit = {
        id: `MS-${u.id}`,
        sierraCode: code,
        type: propertyType,
        compound,
  ...
  // Group by Compound
  const compoundCounts: Record<string, { total: number; sale: number; rent: number; owner: number; broker: number; avgPriceM: number }> = {};
  ...
  console.log(`   - Minimum Price  : ${(minSale / 1_000_000).toFixed(2)}M EGP`);
  console.log(`   - Maximum Price  : ${(maxSale / 1_000_000).toFixed(2)}M EGP`);
  console.log(`   - Average Price  : ${(avgSale / 1_000_000).toFixed(2)}M EGP`);
  console.log(`   - Median Price   : ${(medianSale / 1_000_000).toFixed(2)}M EGP`);
