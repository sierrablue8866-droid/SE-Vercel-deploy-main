...
async function migrateData() {
    ...
    if (existingFile && existingFile.endsWith('.json')) {
        ...
        list.forEach((item, idx) => {
            ...
            listingsToInsert.push({
                ...
                source_channel: _optionalChain([item, 'access', _ => _.ago, 'optionalAccess', _2 => _2.includes, 'call', _3 => _3('WhatsApp')]) ? 'whatsapp' : 'google_sheets',
                status: 'active',
            });
        });
    ...
    for (let i = 0; i < finalUniqueListings.length; i += CHUNK_SIZE) {
        ...
        if (insertError) {
            ...
            totalInserted += (_optionalChain([inserted, 'optionalAccess', _4 => _4.length]) || chunk.length);
        }
    }
