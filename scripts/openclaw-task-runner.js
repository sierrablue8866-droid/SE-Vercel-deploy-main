 function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }import * as dotenv from 'dotenv';
...
async function runTask() {
  ...
  if (args.includes('--test') || commandArg === 'test') {
    ...
    console.log(`📚 Total Indexed Memories: ${_optionalChain([memories, 'optionalAccess', _ => _.length]) || 0}`);
  ...
  if (commandArg === 'ingest:mastersheet' || commandArg === 'ingest:sheet') {
    ...
    const units = JSON.parse(fs.readFileSync(MASTER_SHEET_PATH, 'utf-8'));
    console.log(`📦 Loaded ${units.length} master sheet units`);

    const ownerCount = units.filter((u) => _optionalChain([u, 'access', _2 => _2.ownerType, 'optionalAccess', _3 => _3.toLowerCase, 'call', _4 => _4()]) === 'owner').length;
  ...
  if (commandArg === 'ingest:all') {
    ...
    // Step 2: Master sheet
    if (fs.existsSync(MASTER_SHEET_PATH)) {
      const sheetUnits = JSON.parse(fs.readFileSync(MASTER_SHEET_PATH, 'utf-8'));
  ...
  try {
    ...
    if (result.success) {
      const outputText = _optionalChain([(result.data ), 'optionalAccess', _5 => _5.text]) || JSON.stringify(result.data, null, 2);
