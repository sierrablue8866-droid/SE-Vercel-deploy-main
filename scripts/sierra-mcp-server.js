...
rl.on('line', async (line) => {
  ...
  try {
    ...
    if (method === 'tools/call') {
      const toolName = _optionalChain([params, 'optionalAccess', _ => _.name]);
      const toolArgs = _optionalChain([params, 'optionalAccess', _2 => _2.arguments]) || {};
      ...
      try {
        ...
        sendResponse({
          ...
          error: {
            code: -32000,
            message: _optionalChain([err, 'optionalAccess', _3 => _3.message]) || 'Tool execution error'
          }
        });
      }
      return;
