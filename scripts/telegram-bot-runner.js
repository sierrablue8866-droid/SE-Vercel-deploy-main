...
try {
  ...
  if (fs.existsSync(snapshotPath)) {
    ...
    localUnits = Array.isArray(_optionalChain([data, 'optionalAccess', _ => _.units])) ? data.units : [];
  }
} catch (err) {
...
async function getLiveListings() {
  try {
    ...
    if (res.ok) {
      ...
      if (Array.isArray(_optionalChain([data, 'optionalAccess', _2 => _2.listings]))) return data.listings;
    }
  } catch (e2) {
...
async function handleCommand(text, chatId, senderName) {
      ...
      if (data.ok && Array.isArray(data.result)) {
        for (const update of data.result) {
          ...
          if (update.message && update.message.text) {
            const sender = _optionalChain([update, 'access', _3 => _3.message, 'access', _4 => _4.from, 'optionalAccess', _5 => _5.first_name]) || 'User';
